...
rule build_spikein_counts_table:
    input:
        ...
        input_bam_experimental = expand(f"alignment/{{sample}}_{FACTOR}-chipseq-uniquemappers-experimental.bam", sample=[v["control"] for k,v in CHIPS_SISAMPLES.items()]),
        input_bam_spikein = expand(f"alignment/{{sample}}_{FACTOR}-chipseq-uniquemappers-spikein.bam", sample=[v["control"] for k,v in CHIPS_SISAMPLES.items()])
    output:
